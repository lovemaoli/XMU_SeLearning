...
import org.xmu.chatroom.service.TransApi;
...
public class ChatroomController {
    ...
    @CrossOrigin
    Object saveMessage(@RequestParam String uname){
        try {
            chatRoom.getUserMap().get(uname).saveMessage();
            return "success";
        } catch (JMSException exception) {
            exception.printStackTrace();
            return null;
    ...
    @CrossOrigin
    Object broadcastMessage(@RequestBody Message message){
        var userMap = chatRoom.getUserMap();
        var sender = userMap.get(message.getSender());
        try {
            sender.broadcastMessage(message.getContent());
            return "success";
        } catch (JMSException exception) {
            exception.printStackTrace();
            return null;
    ...
    @GetMapping("translate/query")
    @CrossOrigin
    public String baiduTranslate(@RequestParam("query") String query) {
        return transApi.getTransResult(query);
    }


    @PostMapping("translate/changeTranslate")
    @CrossOrigin
    Object broadcastMessage(){
        transApi.changeTranslate();
        return transApi.getDefaultTo();
    }
}
