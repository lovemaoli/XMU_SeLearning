#MIPS程序    冒泡法排序（升序排列，从小到大）                       #先将10个数（8、1、5、2、7、9、6、4、3、10）存放到地址为1024开始的数据存储器中，然后对这10个数进行排序。
...
sort_loop:
 	lw $s3,1024($s0)                     	#$s3=($s0)
	lw $s4,1024($s1)                	#$s4=($s1)
 	slt $t0,$s4,$s3                	#如果$s4<$s3，则置$t0=1；否则，置$t0=0
 	beq $t0,$zero,sort_next   	#如果$t0=0，则转sort_nent       升序排序         从小到大
